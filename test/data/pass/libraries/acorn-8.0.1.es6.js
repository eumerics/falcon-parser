(function (global, factory) {
  ...
  pp$1.parseForStatement = function(node) {
    ...
    if (this.type === types._var || this.type === types._const || isLet) {
      ...
      if ((this.type === types._in || (this.options.ecmaVersion >= 6 && this.isContextual("of"))) && init.declarations.length === 1) {
  ...
  pp$1.parseBlock = function(createNewLexicalScope = true, node = this.startNode(), exitStrict) {
    node.body = [];
    this.expect(types.braceL);
  ...
  pp$1.parseForIn = function(node, init) {
    const isForIn = this.type === types._in;
    this.next();
  ...
  pp$1.parseClassElement = function(constructorAllowsSuper) {
    ...
    const tryContextual = (k, noLineBreak = false) => {
      ...
      if (this.type !== types.parenL && (!noLineBreak || !this.canInsertSemicolon())) return true
  ...
  pp$2.checkLValSimple = function(expr, bindingType = BIND_NONE, checkClashes) {
  ...
  pp$2.checkLValPattern = function(expr, bindingType = BIND_NONE, checkClashes) {
    switch (expr.type) {
    case "ObjectPattern":
  ...
  pp$2.checkLValInnerPattern = function(expr, bindingType = BIND_NONE, checkClashes) {
    switch (expr.type) {
    case "Property":
      // AssignmentProperty has type === "Property"
  ...
  pp$3.parseExprOp = function(left, leftStartPos, leftStartLoc, minPrec, noIn) {
    ...
    if (prec != null && (!noIn || this.type !== types._in)) {
      if (prec > minPrec) {
        ...
        let coalesce = this.type === types.coalesce;
        if (coalesce) {
  ...
  pp$3.parseSubscript = function(base, startPos, startLoc, noCalls, maybeAsyncArrow, optionalChained) {
    ...
    let optional = optionalSupported && this.eat(types.questionDot);
    if (noCalls && optional) this.raise(this.lastTokStart, "Optional chaining cannot appear in the callee of new expressions");

    let computed = this.eat(types.bracketL);
    if (computed || (optional && this.type !== types.parenL && this.type !== types.backQuote) || this.eat(types.dot)) {
      ...
      this.awaitPos = 0;
      this.awaitIdentPos = 0;
      let exprList = this.parseExprList(types.parenR, this.options.ecmaVersion >= 8, false, refDestructuringErrors);
  ...
  pp$3.parseNew = function() {
    ...
    let startPos = this.start, startLoc = this.startLoc, isImport = this.type === types._import;
  ...
  pp$3.parseTemplate = function({isTagged = false} = {}) {
    let node = this.startNode();
    this.next();
    node.expressions = [];
  ...
  pp$3.checkUnreserved = function({start, end, name}) {
  ...
  pp$8.regexp_eatQuantifier = function(state, noError = false) {
  ...
  pp$8.regexp_eatRegExpUnicodeEscapeSequence = function(state, forceU = false) {
    const start = state.pos;
